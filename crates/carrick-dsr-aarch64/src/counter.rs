//! Darwin commpage/mach-timebase counter planning for the AArch64 DSR lane.
//! Moved verbatim from `carrick-runtime/src/native_darwin/dsr/counter.rs`.
...
///
/// NOTE: deliberately duplicates `carrick-vmm-hvf`'s `trap::sysreg`
/// `host_counter_frequency` (two EL0 `mrs` instructions). The alternative --
/// moving the function here and re-exporting it from the HVF crate -- would
/// point the VMM crate's dependency arrow at the DSR arch crate for one asm
/// read; the duplication is the smaller wart and keeps both darwin
...
pub const COMMPAGE_TIMEBASE_ADDRESS: u64 = 0x0000_000f_ffff_c088;
// Consumed by the macos/aarch64 `host_commpage_mode` arm only.
#[cfg_attr(
...
impl CounterScale {
    ...
    pub fn apply(self, ticks: u64) -> u64 {
...
// Consumed by the macos/aarch64 `host_counter_scale` arm (and by tests on
// every host).
#[cfg_attr(
    not(all(target_os = "macos", target_arch = "aarch64")),
    allow(dead_code)
...
pub fn plan_for_mode(mode: Option<u8>, host_scale: Option<CounterScale>) -> HostCounterPlan {
    match mode {
...
pub const fn counter_word(source: HostCounterSource, destination: u32) -> Option<u32> {
...
#[allow(deprecated)]
pub fn host_counter_scale() -> Option<CounterScale> {
    #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
    {
        ...
        *SCALE.get_or_init(|| {
            ...
            let frequency = host_counter_frequency();
...
pub fn host_counter_plan() -> HostCounterPlan {
    #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
    {
...
#[cfg(target_os = "macos")]
pub fn fallback_counter_ticks() -> Option<u64> {
...
mod tests {
    ...
    fn host_scale_converts_mach_ticks_into_cntfrq_domain() {
        ...
        let before = carrick_host::clock::host_clock_uptime_ns();
        let ticks = scale.apply(mach_absolute_time_ticks());
        let after = carrick_host::clock::host_clock_uptime_ns();
