...
pub struct ProcessTranslator {
    ...
    /// `state.executable_ranges` once at construction
    /// (see `gateway::ExecutableRangeCatalogAuthority`), which keeps the
    /// gateway entry — the hottest path there is — off the process-state
    /// lock while still publishing the catalog the signal handler consults.
...
impl ProcessTranslator {
    ...
    fn new_with_host(
        ...
        // the process state below does not move the header, so the authority
        // copied here stays valid for the process's whole life.
...
impl ThreadTranslator {
    ...
    pub fn enter_prepared<const PROFILE: bool>(
        ...
        // entries, so every prepared entry takes the executable-authority
        // arm with the private record — the shared cache-range/binding-table
        // gateway arms are gone. The entry also publishes the process's
        // executable-range catalog (sole node: the private cache range) so
        // the signal handler classifies interrupted PCs against the catalog
