//! SP3.1 fixture: installs a SA_SIGINFO SIGUSR1 handler, `raise()`s the signal,
//! and verifies (a) the handler ran with the correct `siginfo.si_signo`, and
//! (b) execution RESUMED here after `rt_sigreturn`. Static x86_64-unknown-linux-
//! musl (non-PIE ET_EXEC). Under carrick-bhyve it proves the x86 rt_sigframe
//! round-trip: build_sigframe writes the frame + enters the handler (RDI=signum,
//! RSI=&siginfo, RDX=&ucontext, RIP=handler); the handler returns through musl's
//! `__restore_rt` → rt_sigreturn; restore_from_sigframe restores the pre-signal
//! state so `main` continues.
use std::sync::atomic::{AtomicI32, Ordering};

/// Packs the handler's `sig` arg (low 16) and `info->si_signo` (high 16).
