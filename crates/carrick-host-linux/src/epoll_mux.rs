...
//!
//! There is deliberately NO process-exit class: host-process child waiting
//! belonged to the retired 1:1 lane, and under HVPatch a guest `fork` creates no
//! host process to watch.
//!
//! Every helper fd (`eventfd`/`timerfd`/`inotify`) is owned by this struct and
//! closed on `Drop`; the epoll fd itself is closed last.
