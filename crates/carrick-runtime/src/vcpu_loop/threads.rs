...
fn finish_compatibility_clone_failure(
    rollback: Result<(), carrick_guest_mem::MemoryError>,
    cleanup: impl FnOnce() -> Result<(), RuntimeError>,
) -> Result<(), RuntimeError> {
    cleanup()?;
    rollback.map_err(|error| {
        RuntimeError::Configuration(format!("restore compatibility clone TID outputs: {error}"))
    })
}
...
mod clone_tid_output_tests {
    ...
    #[test]
    fn compatibility_cleanup_completes_before_rollback_error_propagates() {
        let cleaned = std::sync::atomic::AtomicUsize::new(0);
        let rollback = Err(carrick_guest_mem::MemoryError::HostMap(
            "injected rollback failure".to_owned(),
        ));
        let result = finish_compatibility_clone_failure(rollback, || {
    ...
    E::SiblingSpec: 'static,
{
    ...
    pub(super) fn spawn_clone_thread(
        ...
        let tid_outputs =
            match CloneTidOutputTransaction::capture(engine, parent_tid_addr, child_tid_addr) {
                Ok(outputs) => outputs,
                Err(errno) => return Ok(CloneThreadSpawn::Errno(errno)),
            };
        ...
        let host_completion = continuation::LogicalJobCompletion::pending();
        let published_host_completion = host_completion.clone();
        let handle = std::thread::Builder::new()
            .name(host_thread_name)
            .spawn(move || {
                let _host_completion = continuation::LogicalJobCompletionGuard::new(
                    published_host_completion,
                );
                crate::probes::mn_clone_outcome(
                    tid.raw(),
        ...
        if !tid_outputs.publish(engine, linux_tid, tid) {
            let rollback = tid_outputs.rollback(engine);
            finish_compatibility_clone_failure(rollback, || {
                let _ = start_tx.send(false);
                let _ = handle.join();
                if prepared_thread.is_none() {
                    self.registry.exit(tid);
        ...
        self.threads.lock().push(VcpuThreadHandle::Host {
            handle,
            completion: host_completion,
        });
    ...
    pub(super) async fn terminate_siblings_for_exec(
        &mut self,
        kernel: &Kernel,
        engine: &mut E,
    ) -> Result<(), RuntimeError> {
