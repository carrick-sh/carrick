...
/// and that single assignment is what lets the phase-zero classifier answer
/// from the catalog rather than only the installed authority range. Nothing
/// else observes it: the C-level classifier test builds its own context, so
/// reverting the assignment to `null()` would break only this test with every
/// other suite green.
///
