...
// The USDT probe projections moved OUT of the extracted `types` into
// runtime-side extension traits (the arch crate must stay usdt-free); import
// them so the existing `exit.probe_fields()` / `error.probe_outcome()`
// method calls keep resolving.
use types::{DsrErrorProbeExt as _, NativeDsrExitProbeExt as _};
...
// `recovery_resume_pc` / `recover_rewrite_state` moved to
// `carrick_dsr_aarch64::emit` with the emitter whose `RecoveryAction`s they
// interpret; imported so `super::recover_rewrite_state` call paths (mod.rs
// and the oracle) resolve unchanged.
