...
mod tests {
    ...
    mod exclusive_region_emission {
        ...
        fn biased_fused_region_inserts_no_nzcv_writing_word() {
            ...
            // Every remaining word is DSR-inserted by the private-gateway
            // policy. It must remain flag-neutral because biased-exclusive
            // recovery rolls back only its two scratch GPRs. Authority-aware
            // direct-binding stubs are now emitted only while recording a
            // portable unit and have their own focused recovery tests in
            // `carrick-dsr-aarch64`.
            ...
            for (index, &word) in words.iter().enumerate() {
                ...
                assert!(
                    ...
                    "private DSR-inserted word 0x{word:08x} at +{:#x} \
                     ({instruction}) writes NZCV under biased-exclusive two-GPR recovery",
                    index * 4
                );
            }
            assert!(
