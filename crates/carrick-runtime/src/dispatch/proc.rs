...
syscall_table! {
    ...
    nr::IOPRIO_SET => ioprio_set,
    nr::IOPRIO_GET => ioprio_get,
    nr::VHANGUP => vhangup,
    nr::PERSONALITY => personality,
    nr::WAITID => waitid,
    ...
    nr::SETDOMAINNAME => setdomainname,
    nr::PRCTL => prctl,
    nr::GETCPU => getcpu,
    nr::CLONE => clone,
    nr::EXECVE => execve,
    nr::FUTEX_WAITV => futex_waitv,
    ...
    nr::PIDFD_SEND_SIGNAL => pidfd_send_signal,
    nr::PIDFD_OPEN => pidfd_open,
    nr::PIDFD_GETFD => pidfd_getfd,
    nr::EXIT | nr::EXIT_GROUP => sys_exit,
    nr::GETTID => gettid,
    nr::CLONE3 => sys_clone3,
    nr::RSEQ => sys_rseq,
}
...
impl<'a> ProcView<'a> {
    define_syscall! {
        ...
        fn ptrace(this, cx, request: u64, pid: Pid, addr: GuestPtr, data: u64) {
            ...
            // A task with no HVPatch process binding has no tracee: the
            // retired 1:1 native lane's host `ptrace` calls lived here
            // (e1fbfd32e) and were deleted with the kernel extraction plan;
            // git remembers them.
