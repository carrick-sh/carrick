...
/// unrelated process or namespace. The type is linear and borrow-bound to the exact
/// lifetime of the process guard that minted it.
...
mod tests {
    ...
    #[test]
    fn paired_lock_authority_operates_on_live_dispatcher() {
        let dispatcher = SyscallDispatcher::new();
        let mut process = dispatcher.lock_sysv_process();
        assert_eq!(process.first_remapped_attachment(), None);
        assert!(process.is_inheritance_committed());

        let permit = process.namespace_permit();
        let paired = permit.lock_paired();
        assert!(paired.state.segments.is_empty());
        drop(paired);
    }
}
