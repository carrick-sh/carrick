...
use carrick_runtime::linux_abi::{
    ...
    LINUX_SOCK_CLOEXEC, LINUX_SOCK_NONBLOCK, LINUX_SOCK_STREAM, LINUX_SOL_TCP,
};
#[cfg(target_os = "macos")]
...
fn close_of_added_fd_auto_removes_it_from_epoll_interest() {
    ...
    assert_eq!(
        call(
            ...
            &mut memory,
            21,
            [epfd, LINUX_EPOLL_CTL_ADD, efd, 0x4040, 0, 0]
        ),
        DispatchOutcome::Returned { value: 0 }
    );
    ...
    assert_eq!(
        efd2, efd,
        "closed fd number should be reused by the allocator"
    );
    ...
    assert_eq!(
        call(
            ...
            &mut memory,
            21,
            [epfd, LINUX_EPOLL_CTL_ADD, efd2, 0x4040, 0, 0]
        ),
        DispatchOutcome::Returned { value: 0 },
