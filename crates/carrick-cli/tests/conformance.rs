...
const PROBE_SOURCE_COUNT: usize = 502;
...
fn closure_probe_inventory_enforces_authoritative_runners_and_denominator() {
    ...
    assert_eq!(generic.len(), 453);
    assert_eq!(generic.len() + DEDICATED_PROBE_RUNNERS.len(), 476);
    assert_eq!(2 * (generic.len() + DEDICATED_PROBE_RUNNERS.len()), 952);
