...
mod alias_registry_tests {
    ...
    #[test]
    fn live_alias_backings_walk_host_regions_once_and_preserve_publication_order() {
        let base = AliasBacking {
            start: 0x4000,
            ipa: 0x8000,
            host_addr: 0,
            size: 0x100,
        ...
        let (live, dead) = partition_live_alias_backings_with(&aliases, |address| {
            queries.push(address);
            match address {
                0x1000 => Some((0x1000, 0x2000)),
                0x2800 => Some((0x2800, 0x3000)),
                0x5000 => None,
                unexpected => panic!("unexpected region query at {unexpected:#x}"),
            }
        });

        assert_eq!(queries, vec![0x1000, 0x2800, 0x5000]);
        assert_eq!(
            live.iter().map(|alias| alias.host_addr).collect::<Vec<_>>(),
            vec![0x2800, 0x1000, 0x1800],
            "batched liveness must preserve the registry's publication order"
        );
        assert_eq!(
            dead.iter().map(|alias| alias.host_addr).collect::<Vec<_>>(),
            vec![0, 0x5000]
        );
    }

    #[test]
    fn fork_alias_liveness_checks_only_stage1_selected_publications() {
        let selected = AliasBacking {
            start: 0x4000,
            ipa: 0x8000,
            host_addr: 0x1000,
            size: 0x100,
        ...
        let stale = AliasBacking {
            start: 0x8000,
            ipa: 0xc000,
            host_addr: 0x1800,
            ..selected
        };
        let dead = AliasBacking {
            start: 0xc000,
            ipa: 0x10_000,
            host_addr: 0x5000,
            ..selected
        ...
        let (live, dead_current) = partition_current_live_alias_backings_with(
            &[stale, selected, dead],
            |va| match va {
                0x4000 => Some(0x8000),
                0x8000 => Some(0xd000),
                0xc000 => Some(0x10_000),
                _ => None,
            },
            |address| {
                ...
                match address {
                    0x1000 => Some((0x1000, 0x2000)),
                    0x5000 => None,
                    unexpected => panic!("stage-1-stale alias queried at {unexpected:#x}"),
                }
            },
        );

        assert_eq!(live, vec![selected]);
        assert_eq!(dead_current, vec![dead]);
        assert_eq!(queried, vec![0x1000, 0x5000]);
    }
