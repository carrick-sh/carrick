...
fn extracts_file_dir_symlink_with_mode() {
    ...
    let stats = carrick_vfs::rootfs::extract_layer_paths_to_dir(&[layer], scratch.path()).unwrap();
...
fn later_layer_overrides_and_whiteout_deletes() {
    ...
    carrick_vfs::rootfs::extract_layer_paths_to_dir(&[l0, l1], scratch.path()).unwrap();
    ...
    let layer_path = tmp.path().join("escape.tar");
    {
        ...
        match h.set_path("../evil") {
            Ok(_) => {
                ...
                let result = carrick_vfs::rootfs::extract_layer_paths_to_dir(
                    &[layer_path.clone()],
                    scratch.path(),
                );
