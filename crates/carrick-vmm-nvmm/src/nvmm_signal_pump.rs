//! NVMM async host-signal pump — now a thin forwarder to the SHARED
//! [`carrick_hal::signal_pump`], parameterized by [`crate::NvmmGlue`]. The pump
//! logic is byte-identical across KVM/bhyve/NVMM and lives in carrick-hal; this
//! module keeps NVMM's call-site API stable. (The runtime calls the shared pump
...
/// Start the async host-signal pump (idempotent), installing `NvmmGlue` handlers.
pub fn start_pump(registry: &Arc<dyn VcpuRegistry>, futex: &Arc<dyn PlatformFutex>) {
    pump::start_pump::<NvmmGlue>(registry, futex);
}
...
pub fn reinit_after_fork(registry: &Arc<dyn VcpuRegistry>, futex: &Arc<dyn PlatformFutex>) {
    pump::reinit_after_fork::<NvmmGlue>(registry, futex);
}
