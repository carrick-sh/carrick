...
pub struct PreparedGuestVa(u64);

impl PreparedGuestVa {
    pub fn new(value: u64) -> Option<Self> {
        (value <= carrick_mem::memory::LINUX_STACK_TOP).then_some(Self(value))
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct PreparedGuestLen(u64);
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct PreparedArtifactOffset(u64);
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct PreparedRegionIndex(u16);
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct NativeRelativeRelocation {
    address: PreparedGuestVa,
    value: PreparedGuestVa,
}

impl NativeRelativeRelocation {
    pub fn new(address: PreparedGuestVa, value: PreparedGuestVa) -> Self {
        Self { address, value }
    }

    pub fn address(self) -> PreparedGuestVa {
        self.address
    }

    pub fn value(self) -> PreparedGuestVa {
        self.value
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct NativePreparedSpanV1 {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct NativePreparedRoSpanV1 {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct NativePreparedImageV1 {
    artifact_fd: RawFd,
...
#[derive(Debug)]
// Unconditional on purpose (the arch crate compiles on every target; the
// memory model embeds this type in its prepared-image mapping paths). The
// capsule VALIDATION that produces it (`validate_for_resume`) stays
...
impl ValidatedPreparedImage {
    ...
    pub fn into_image(self) -> AddressSpace {
        self.image
    }
}
...
pub enum PreparedImageIneligibleReason {
    SharedRegion {
        index: usize,
    },
    RepresentationLimit {
...
pub enum NativePreparedImageError {
    ...
    AddressSpace(#[from] carrick_mem::memory::AddressSpaceError),
}
...
pub fn native_region_copy_window(
    region: &MemoryRegion,
    ...
    if region.start != stack_start || region.end != carrick_mem::memory::LINUX_STACK_TOP {
        return full;
    }
...
fn validate_record(
    ...
    let stack_region_index = record
        ...
        .position(|region| {
            region.guest_start.get() == stack_start
                && region.guest_end.get() == carrick_mem::memory::LINUX_STACK_TOP
        })
    ...
    if stack_region.permissions & 0b010 == 0
        ...
        || record.initial_stack_pointer.get() >= carrick_mem::memory::LINUX_STACK_TOP
    {
        return Err(validation(
...
mod tests {
    ...
    use carrick_mem::memory::{AddressSpace, LINUX_STACK_SIZE, LINUX_STACK_TOP};
    ...
    fn prepared_with_coverage(coverage: ArtifactDigestCoverage) -> PreparedImageArtifact {
        match prepare_with_coverage(&synthetic_image(), &relocations(), HOST_PAGE_SIZE, coverage)
            .expect("prepare sparse artifact")
        {
            PreparedImageDisposition::Prepared(artifact) => artifact,
            PreparedImageDisposition::Ineligible(reason) => {
    ...
    fn prepared_image_limits_are_ineligible_at_construction_and_invalid_on_import() {
        ...
        let shared = AddressSpace::from_metadata(carrick_mem::memory::AddressSpaceMetadata {
            ...
            regions: vec![carrick_mem::memory::MemoryRegionMetadata {
        ...
        let regions = (0..257_u64)
            .map(|index| {
                ...
                carrick_mem::memory::MemoryRegionMetadata {
        ...
        let too_many_source_regions =
            AddressSpace::from_metadata(carrick_mem::memory::AddressSpaceMetadata {
