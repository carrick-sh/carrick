...
pub const MUSL_SHARD_1_EXPECTED_GAPS: &[&str] = &["eventwaitmatrix", "vfs_mount_rw"];

/// Shard 1 subset of baseline expected oracle mismatches for gnu.
pub const GNU_SHARD_1_EXPECTED_GAPS: &[&str] = &["eventwaitmatrix", "vfs_mount_rw"];
...
fn test_shard_1_inventory_count_and_sorted() {
    assert_eq!(
        SHARD_1_PROBES.len(),
        145,
        "shard 1 must contain exactly 145 generic probes"
    );
    ...
    if let Ok(content) = std::fs::read_to_string(&inventory_path) {
        ...
        assert_eq!(
            ...
            145,
            "computed shard 1 from probe-inventory.json must have 145 items"
        );
        assert_eq!(
            SHARD_1_PROBES,
