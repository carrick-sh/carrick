...
const CACHED_SHARD_0_PROBE_COUNT: usize = 148;
...
fn test_shard_0_inventory() {
    // Hard-assert exactly 157 sorted unique names.
    assert_eq!(
        SHARD_0_PROBES.len(),
        157,
        "shard 0 must have exactly 157 probes"
    );
    ...
    assert_eq!(
        unique_probes.len(),
        157,
        "SHARD_0_PROBES must contain 157 unique names"
    );
    assert_eq!(
        SHARD_0_PROBES
            .iter()
    ...
    assert_eq!(
        selected_names.len(),
        471,
        "expected exactly 471 conformance generic probes in inventory"
    );
    ...
    assert_eq!(
        derived_shard_0.len(),
        157,
        "derived shard 0 must have exactly 157 items"
    );
    assert_eq!(
        SHARD_0_PROBES,
