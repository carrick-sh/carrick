...
fn test_shard_0_inventory() {
    // Hard-assert exactly 164 sorted unique names.
    assert_eq!(
        SHARD_0_PROBES.len(),
        164,
        "shard 0 must have exactly 164 probes"
    );
    ...
    assert_eq!(
        unique_probes.len(),
        164,
        "SHARD_0_PROBES must contain 164 unique names"
    );
    assert_eq!(
        SHARD_0_PROBES
            .iter()
    ...
    assert_eq!(
        selected_names.len(),
        491,
        "expected exactly 491 conformance generic probes in inventory"
    );
    ...
    assert_eq!(
        derived_shard_0.len(),
        164,
        "derived shard 0 must have exactly 164 items"
    );
    assert_eq!(
        SHARD_0_PROBES,
