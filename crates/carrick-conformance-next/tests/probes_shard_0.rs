...
fn test_shard_0_inventory() {
    // Hard-assert exactly 161 sorted unique names.
    assert_eq!(
        SHARD_0_PROBES.len(),
        161,
        "shard 0 must have exactly 161 probes"
    );
    ...
    assert_eq!(
        unique_probes.len(),
        161,
        "SHARD_0_PROBES must contain 161 unique names"
    );
    assert_eq!(
        SHARD_0_PROBES
            .iter()
    ...
    assert_eq!(
        selected_names.len(),
        481,
        "expected exactly 481 conformance generic probes in inventory"
    );
    ...
    assert_eq!(
        derived_shard_0.len(),
        161,
        "derived shard 0 must have exactly 161 items"
    );
    assert_eq!(
        SHARD_0_PROBES,
