...
const CACHED_SHARD_0_PROBE_COUNT: usize = 147;
...
fn test_shard_0_inventory() {
    // Hard-assert exactly 160 sorted unique names.
    assert_eq!(
        SHARD_0_PROBES.len(),
        160,
        "shard 0 must have exactly 160 probes"
    );
    ...
    assert_eq!(
        unique_probes.len(),
        160,
        "SHARD_0_PROBES must contain 160 unique names"
    );
    assert_eq!(
        SHARD_0_PROBES
            .iter()
    ...
    assert_eq!(
        selected_names.len(),
        479,
        "expected exactly 479 conformance generic probes in inventory"
    );
    ...
    assert_eq!(
        derived_shard_0.len(),
        160,
        "derived shard 0 must have exactly 160 items"
    );
    assert_eq!(
        SHARD_0_PROBES,
