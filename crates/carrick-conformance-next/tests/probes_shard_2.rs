...
fn test_shard_2_inventory_definition() {
    // 1. Hard-assert exactly 156 sorted unique names.
    assert_eq!(
        SHARD_2_PROBES.len(),
        156,
        "shard 2 must contain exactly 156 probes"
    );
    let probe_set: BTreeSet<&str> = SHARD_2_PROBES.iter().copied().collect();
    assert_eq!(
        probe_set.len(),
        156,
        "SHARD_2_PROBES must contain 156 unique names"
    );
    ...
    assert_eq!(
        ...
        469,
        "expected exactly 469 generic conformance probes across all shards"
    );
    ...
    assert_eq!(
        derived_shard_2.len(),
        156,
        "derived shard 2 must have 156 items"
    );
    assert_eq!(
