...
fn test_shard_2_inventory_definition() {
    // 1. Hard-assert exactly 155 sorted unique names.
    assert_eq!(
        SHARD_2_PROBES.len(),
        155,
        "shard 2 must contain exactly 155 probes"
    );
    let probe_set: BTreeSet<&str> = SHARD_2_PROBES.iter().copied().collect();
    assert_eq!(
        probe_set.len(),
        155,
        "SHARD_2_PROBES must contain 155 unique names"
    );
    ...
    assert_eq!(
        ...
        465,
        "expected exactly 465 generic conformance probes across all shards"
    );
    ...
    assert_eq!(
        derived_shard_2.len(),
        155,
        "derived shard 2 must have 155 items"
    );
    assert_eq!(
