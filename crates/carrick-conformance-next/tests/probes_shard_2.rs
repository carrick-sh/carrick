...
/// Hard-asserted to have exactly 145 sorted unique names.
...
fn test_shard_2_inventory_definition() {
    // 1. Hard-assert 145 sorted unique names.
    assert_eq!(
        SHARD_2_PROBES.len(),
        145,
        "shard 2 must contain exactly 145 probes"
    );
    ...
    assert_eq!(
        ...
        436,
        "expected exactly 436 generic conformance probes across all shards"
    );
    ...
    assert_eq!(
        derived_shard_2.len(),
        145,
        "derived shard 2 must have 145 items"
    );
    assert_eq!(
