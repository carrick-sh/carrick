...
const CACHED_SHARD_2_PROBE_COUNT: usize = 143;
...
fn test_shard_2_inventory_definition() {
    // 1. Hard-assert exactly 159 sorted unique names.
    assert_eq!(
        SHARD_2_PROBES.len(),
        159,
        "shard 2 must contain exactly 159 probes"
    );
    let probe_set: BTreeSet<&str> = SHARD_2_PROBES.iter().copied().collect();
    assert_eq!(
        probe_set.len(),
        159,
        "SHARD_2_PROBES must contain 159 unique names"
    );
    ...
    assert_eq!(
        ...
        477,
        "expected exactly 477 generic conformance probes across all shards"
    );
    ...
    assert_eq!(
        derived_shard_2.len(),
        159,
        "derived shard 2 must have 159 items"
    );
    assert_eq!(
