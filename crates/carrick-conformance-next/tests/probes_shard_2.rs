...
const CACHED_SHARD_2_PROBE_COUNT: usize = 147;
...
fn test_shard_2_inventory_definition() {
    // 1. Hard-assert exactly 162 sorted unique names.
    assert_eq!(
        SHARD_2_PROBES.len(),
        162,
        "shard 2 must contain exactly 162 probes"
    );
    let probe_set: BTreeSet<&str> = SHARD_2_PROBES.iter().copied().collect();
    assert_eq!(
        probe_set.len(),
        162,
        "SHARD_2_PROBES must contain 162 unique names"
    );
    ...
    assert_eq!(
        ...
        488,
        "expected exactly 488 generic conformance probes across all shards"
    );
    ...
    assert_eq!(
        derived_shard_2.len(),
        162,
        "derived shard 2 must have 162 items"
    );
    assert_eq!(
