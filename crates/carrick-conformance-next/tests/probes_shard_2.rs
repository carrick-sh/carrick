...
fn test_shard_2_inventory_definition() {
    // 1. Hard-assert exactly 161 sorted unique names.
    assert_eq!(
        SHARD_2_PROBES.len(),
        161,
        "shard 2 must contain exactly 161 probes"
    );
    let probe_set: BTreeSet<&str> = SHARD_2_PROBES.iter().copied().collect();
    assert_eq!(
        probe_set.len(),
        161,
        "SHARD_2_PROBES must contain 161 unique names"
    );
    ...
    assert_eq!(
        ...
        484,
        "expected exactly 484 generic conformance probes across all shards"
    );
    ...
    assert_eq!(
        derived_shard_2.len(),
        161,
        "derived shard 2 must have 161 items"
    );
    assert_eq!(
