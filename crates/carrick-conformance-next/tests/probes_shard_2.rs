...
const CACHED_SHARD_2_PROBE_COUNT: usize = 151;
...
fn test_shard_2_inventory_definition() {
    // 1. Hard-assert exactly 160 sorted unique names.
    assert_eq!(
        SHARD_2_PROBES.len(),
        160,
        "shard 2 must contain exactly 160 probes"
    );
    let probe_set: BTreeSet<&str> = SHARD_2_PROBES.iter().copied().collect();
    assert_eq!(
        probe_set.len(),
        160,
        "SHARD_2_PROBES must contain 160 unique names"
    );
    ...
    assert_eq!(
        ...
        481,
        "expected exactly 481 generic conformance probes across all shards"
    );
    ...
    assert_eq!(
        derived_shard_2.len(),
        160,
        "derived shard 2 must have 160 items"
    );
    assert_eq!(
