...
extern "C" fn on_rt(_sig: libc::c_int, info: *mut libc::siginfo_t, _ctx: *mut libc::c_void) {
    ...
    CODE.store(
        unsafe { std::ptr::read_unaligned(p.add(8) as *const i32) },
        Ordering::SeqCst,
    );
    VAL.store(
        unsafe { std::ptr::read_unaligned(p.add(24) as *const i64) },
        Ordering::SeqCst,
    );
}
fn main() {
    unsafe {
        ...
        if pid == 0 {
            ...
            let mut i = 0;
            while VAL.load(Ordering::SeqCst) == -1 && i < 3000 {
                libc::usleep(1000);
                i += 1;
            }
            ...
            println!(
                "rt_code={} rt_value=0x{:x} correct={}",
                c,
                v,
                ((v == PAYLOAD) && (c == -1)) as i32
            );
            libc::_exit(0);
        }
        let mut b = [0u8; 1];
