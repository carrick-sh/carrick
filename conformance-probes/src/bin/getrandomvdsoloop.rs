...
unsafe fn r16(p: u64) -> u16 {
    ptr::read_unaligned(p as *const u16)
}
unsafe fn r32(p: u64) -> u32 {
    ptr::read_unaligned(p as *const u32)
}
unsafe fn r64(p: u64) -> u64 {
    ptr::read_unaligned(p as *const u64)
}
unsafe fn ri64(p: u64) -> i64 {
    ptr::read_unaligned(p as *const i64)
}
unsafe fn ceq(p: u64, w: &str) -> bool {
    for (i, &b) in w.as_bytes().iter().enumerate() {
        if *((p + i as u64) as *const u8) != b {
            return false;
        }
    }
    *((p + w.len() as u64) as *const u8) == 0
}
unsafe fn sym(w: &str) -> u64 {
    let b = libc::getauxval(AT_SYSINFO_EHDR);
    if b == 0 {
        return 0;
    }
    let ph0 = r64(b + 0x20);
    let es = r16(b + 0x36) as u64;
    let n = r16(b + 0x38) as u64;
    let mut dy = 0u64;
    for i in 0..n {
        let ph = b + ph0 + i * es;
        if r32(ph) == 2 {
    ...
    let (mut st, mut str_, mut h) = (0u64, 0u64, 0u64);
    let mut d = dy;
    loop {
        let t = ri64(d);
        let v = r64(d + 8);
    ...
    let nc = if h != 0 { r32(h + 4) } else { 0 };
    if st == 0 || str_ == 0 {
        return 0;
    }
    for s in 0..nc as u64 {
        let sy = st + s * 24;
        let nm = r32(sy) as u64;
        let sh = r16(sy + 6);
        let val = r64(sy + 8);
        if nm == 0 || sh == 0 {
            continue;
...
fn main() {
    unsafe {
        ...
        let mut buf = [0u8; 16];
        for _ in 0..200 {
            let n = f(buf.as_mut_ptr(), 16, 0, st as *mut u8, p.size as usize);
            if n == 16 {
                total += 16
            }
        }
