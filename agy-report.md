...
3. **Identity-mapped page tables with FEAT_PAN3 workaround.** The stage-1 identity mapping ([memory.rs](file:///path/to/carrick/crates/carrick-mem/src/memory.rs)) that enables the MMU for guest `ldaxr`/`stlxr` atomics is elegant engineering. The PAN bypass via AP/PXN bits is a clever solution to an Apple Silicon-specific constraint.
...
The dispatch architecture in [dispatch/mod.rs](file:///path/to/carrick/crates/carrick-runtime/src/dispatch/mod.rs) is well-structured:
...
### File Descriptor Table ([fd_table.rs](file:///path/to/carrick/crates/carrick-runtime/src/dispatch/fd_table.rs))
...
### /proc Synthesis ([vfs/proc.rs](file:///path/to/carrick/crates/carrick-runtime/src/vfs/proc.rs) — 918 lines)
...
### OCI Layer Composition ([rootfs.rs](file:///path/to/carrick/crates/carrick-runtime/src/rootfs.rs) — 824 lines)

- Streams tar layers, handles `.wh.<name>` file whiteouts and `.wh..wh..opq` opaque directory whiteouts
- `HostFsBackend` uses `clonefile(2)` (APFS copy-on-write) for near-instant layer reuse via [layer_cache.rs](file:///path/to/carrick/crates/carrick-runtime/src/layer_cache.rs)
...
### Page Table Manager ([page_table.rs](file:///path/to/carrick/crates/carrick-mem/src/page_table.rs) — 845 lines)
...
### EL1 Plumbing ([memory.rs](file:///path/to/carrick/crates/carrick-mem/src/memory.rs))
...
> [!IMPORTANT]
> The lock ordering documented at the top of [dispatch/mod.rs](file:///path/to/carrick/crates/carrick-runtime/src/dispatch/mod.rs#L11-L18) is exactly what Solaris does with its mutex-ordering comments, and it's critical for deadlock prevention.
...
### FutexTable ([thread.rs](file:///path/to/carrick/crates/carrick-runtime/src/thread.rs))
...
### Fork Quiesce ([fork_quiesce.rs](file:///path/to/carrick/crates/carrick-runtime/src/fork_quiesce.rs) + [runtime.rs](file:///path/to/carrick/crates/carrick-runtime/src/runtime.rs#L1323-L1519))
...
**Signal frame injection** (`inject_signal` / `restore_from_sigframe` in [trap.rs](file:///path/to/carrick/crates/carrick-runtime/src/trap.rs)):
...
### DevPTS ([vfs/devpts.rs](file:///path/to/carrick/crates/carrick-runtime/src/vfs/devpts.rs))
...
### Three-Stage Pipeline ([carrick-image](file:///path/to/carrick/crates/carrick-image/src/lib.rs) — 511 lines)
...
### Engine ([carrick-engine](file:///path/to/carrick/crates/carrick-engine/src) — ~337 lines)
...
### USDT/DTrace Probes ([probes.rs](file:///path/to/carrick/crates/carrick-runtime/src/probes.rs) — 457 lines)
...
### In-Process DTrace Consumer ([dtrace_consumer.rs](file:///path/to/carrick/crates/carrick-runtime/src/dtrace_consumer.rs) — 523 lines)
...
### Compat Reporter ([compat.rs](file:///path/to/carrick/crates/carrick-runtime/src/compat.rs) — 621 lines)
