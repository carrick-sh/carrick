...
- Stay in the carrier: `vcpu_loop/continuation/quantum.rs` (1,587 lines; `crate::hvpatch` ×19, `crate::trap` ×42 — executor quantum accounting). **Ruling 2026-09-16 (Task 2.2 escalation E1):** `vcpu_loop/continuation/readiness.rs` moves in THIS task to `kernel/continuation/readiness.rs` — its types (`ReadinessProbe`, `ReservedSignal`, `StaleThreadCause`, `ContinuationWakeToken`, `ResumeContext`, `SignalReadinessProbe`) are fields and parameters of the continuation model and wait service (type-level, bidirectional coupling; 113 shared `pub(in crate::vcpu_loop)` items), so leaving it behind makes `kernel/continuation.rs` import upward. Its four `crate::host_signal` calls (`SignalReadinessProbe::event`, ~L470-509) are the exact measured residue Task 2.5's `HostSignalBridge` removes; Step 5's grep for this task therefore tolerates exactly those four `crate::host_signal` lines and nothing else. `pub(in crate::vcpu_loop)` on moved items becomes `pub(crate)`. `vcpu_loop/continuation/tests.rs` (6,555 lines) is split by the same rule: tests of moved code move, tests naming `hvpatch`/`host_signal`/executor internals stay.
...
- [ ] **Step 5: Verify.** `grep -rn "crate::vcpu_loop\|carrick_vmm_hvf" crates/carrick-runtime/src/kernel` → no output; `grep -rn "crate::host_signal" crates/carrick-runtime/src/kernel` → exactly the four `SignalReadinessProbe::event` lines in `kernel/continuation/readiness.rs` (Task 2.5's residue). `kernel/mm_access.rs`'s `cow_fixture` (a shared fixture, not a test) may keep its `Stage1MmPool::new_root_for_tests` call until Task 2.3 Step 3b gives it the test projection (ruling on escalation E2).
...
- [ ] **Step 3b (from Task 2.15's landing):** `kernel::carrier_process::TestCarrierProcess` currently holds a REAL `Stage1MmLease` from `Stage1MmPool::new_root` (kernel/carrier_process.rs ~L794-804, 866) because `bind_hvpatch_process_exact` (dispatch/kernel_context.rs ~L126-134) builds the foreign-mm mutation authority from the lease and fatals without one. Give the double a `TestStage1MmProjection` (kernel-side, `#[cfg(test)]`) implementing `Stage1MmProjection` with the minimal behaviour those 26 fixtures need, and give `kernel/mm_access.rs`'s shared `cow_fixture` the same test projection (it only names `Stage1MmPool::new_root_for_tests` to build a `ForeignMmMutationAuthority`, which fatals without a lease — Task 2.2 escalation E2), so that after this task `grep -rn "crate::hvpatch" crates/carrick-runtime/src/kernel crates/carrick-runtime/src/dispatch` is EMPTY. `CarrierProcess::stage1_mm_lease` returns `Arc<dyn Stage1MmProjection>`.
...
- [ ] **Step 5b:** `kernel/continuation/readiness.rs` already moved in Task 2.2 (ruling E1); here its four `crate::host_signal` calls in `SignalReadinessProbe::event` go through `HostSignalBridge`, after which `grep -rn "crate::host_signal" crates/carrick-runtime/src/kernel` is empty.
...
- [ ] **Step 0 (from Task 2.15's landing):** the `CarrierProcess` trait and its doubles (`TestCarrierProcess`, `TestStage1MmProjection`, the promoted `TestMmBackend`) are `pub(crate)` under `#[cfg(test)] mod test_support` because a feature-only build produced dead-code warnings on `pub(crate)` items. In this task they become `pub` (with doc comments — global constraint 5) behind `#[cfg(any(test, feature = "test-support"))]`, and the `expect`s in `TestCarrierProcess::new` move behind a `Result` (clippy `expect_used` only exempts `cfg(test)`); `carrick-kernel-example` (Task 2.12) depends on `carrick-kernel` with `features = ["test-support"]` to build its backend from them. `CarrierProcess::register_pidfd_watch` names `dispatch::fd_table::PidfdWatch` from `kernel/` — fine, both move together.
