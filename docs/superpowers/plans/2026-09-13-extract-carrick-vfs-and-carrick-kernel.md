...
- [ ] **Step 3b (from Task 2.15's landing):** `kernel::carrier_process::TestCarrierProcess` currently holds a REAL `Stage1MmLease` from `Stage1MmPool::new_root` (kernel/carrier_process.rs ~L794-804, 866) because `bind_hvpatch_process_exact` (dispatch/kernel_context.rs ~L126-134) builds the foreign-mm mutation authority from the lease and fatals without one. Give the double a `TestStage1MmProjection` (kernel-side, `#[cfg(test)]`) implementing `Stage1MmProjection` with the minimal behaviour those 26 fixtures need, so that after this task `grep -rn "crate::hvpatch" crates/carrick-runtime/src/kernel crates/carrick-runtime/src/dispatch` is EMPTY except the three `Stage1MmPool::new_root_for_tests` sites Task 2.2 owns (if 2.2 has landed, empty outright). `CarrierProcess::stage1_mm_lease` returns `Arc<dyn Stage1MmProjection>`.
