...
| `dispatch` → `hvpatch` | 59 | `process_context_for_tests` ×26, `ProcessContext` ×14, `WaitResult` ×9, `Stage` ×4, `ProcessThreadExit` ×3, `identity_operation_errno` ×2, `ChildExit` ×1 | Task 2.1 |
...
- Modify: `crates/carrick-hal/src/lib.rs`, `dispatch/mm_authority.rs:180`, `dispatch/mm_mutation.rs:212,222`, `hvpatch/stage1_mm.rs` (impl)
...
- [ ] **Step 2:** Replace the three `Arc<crate::hvpatch::Stage1MmLease>` with `Arc<dyn Stage1MmProjection>`; `impl Stage1MmProjection for Stage1MmLease` in `hvpatch/stage1_mm.rs`, forwarding.
