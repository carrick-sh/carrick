...
./target/release/carrick run
    ...
    <cmd...>
```
**Flag-ordering — the real, code-enforced constraint (corrected during implementation).** In `crates/carrick-cli/src/args.rs` the `Run` variant declares `image: String` as a positional and `command: Vec<String>` with `#[arg(trailing_var_arg = true, allow_hyphen_values = true)]`; `--raw`, `--fs`, `-v`, `-w`, `-e`, `--entrypoint` are ordinary clap options. With `trailing_var_arg`, **the first positional token (the image) terminates option parsing — every token after the image is captured verbatim into `command` and handed to the guest.** Therefore **ALL carrick envelope flags must come BEFORE the image, and only `cmd` after it**: `carrick run <envelope flags> <image> <cmd...>` — identical in shape to `docker run [opts] <image> <cmd>` (and to `scripts/go-conformance-image.sh:53`). Putting `--fs memory` *after* the image would pass `--fs memory` to the guest as argv, not parse it — so the earlier "image then flags" convention was wrong and is not used. `engine.rs` builds `run <carrick_flags> <--entrypoint> <-v> <-w> <-e> <image> <cmd...>` accordingly. (ltp-check.sh's `<image> --raw --fs host` ordering "worked" only because the case-sensitive-volume default is already `host`; the `--raw --fs host` tokens were in fact handed to `/bin/sh`, benign there but wrong in general.)
