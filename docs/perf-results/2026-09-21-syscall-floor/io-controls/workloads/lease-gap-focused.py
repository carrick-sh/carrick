...
for index,arm in enumerate('ABBAABBA'):
    screen.BIN=screen.ROOT/'target/lease-cost'/('carrick-scrub-candidate' if arm=='A' else 'carrick-lease-gap')
    label=f'lease-gap-focused-{index}-{arm}'
...
for name in ['node-app','go-build-cold','python-os']:
    arms={a:[] for a in 'AB'}
    for index,arm in enumerate('ABBAABBA'):
        rows=[json.loads(x) for x in (out/f'lease-gap-focused-{index}-{arm}.jsonl').read_text().splitlines()]
...
(out/'lease-gap-focused-summary.json').write_text(json.dumps(results,indent=2)+'\n')
