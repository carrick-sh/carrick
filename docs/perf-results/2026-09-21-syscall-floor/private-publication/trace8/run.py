...
out=root/'target/lease-cost/private-publication/trace8'
binary=Path('/Volumes/CaseSensitive/carrick/target/syscall-floor/carrick-private-publication')
...
rid='private-publish-shape-20260922'
...
env=os.environ.copy();env.update(CARRICK_FAULT_WINDOW_BYTES='65536',CARRICK_RUN_ID=rid,CARRICK_INSECURE_REGISTRIES='localhost:5005,localhost:5050')
(out/'inputs.json').write_text(json.dumps({'argv':cmd,'run_id':rid,'iterations':1,'host_fault_window_bytes':65536,'binary_sha256':hashlib.sha256(binary.read_bytes()).hexdigest(),'script_sha256':hashlib.sha256(script.read_bytes()).hexdigest()},indent=2)+'\n')
