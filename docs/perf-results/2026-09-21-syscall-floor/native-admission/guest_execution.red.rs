...
mod tests {
    ...
    fn duplicate_admission_preserves_live_pause_endpoint() {
        use carrick_hal::{InGuestFlag, VcpuRegistry, VcpuRegistrationEnrollment};
        use std::sync::atomic::{AtomicUsize, Ordering};

        #[derive(Clone)]
        struct CountKick(Arc<AtomicUsize>);
        impl carrick_hal::VcpuKick for CountKick {
            fn kick(&self) { self.0.fetch_add(1, Ordering::SeqCst); }
        }

        use carrick_conformance_contract::{Completeness, ContractId, ContractObservation,
            ContractRegistry, ExecutionLayer, SemanticAssertion, WorkMetric, WorkSnapshot, evaluate};
        ...
        for scale in [1, 8, 32, 128] {
            ...
            assert!(matches!(registry.subscribe_register(tid,
                Box::new(CountKick(original_kicks.clone())), &running, Arc::new(|| {})),
                VcpuRegistrationEnrollment::Registered));
            assert!(matches!(displaced.subscribe_register(tid,
                Box::new(CountKick(duplicate_kicks.clone())), &idle, Arc::new(|| {})),
                VcpuRegistrationEnrollment::Registered));
            let mut original = census.enter_with_pause_endpoint(
                Some(context.thread().clone()), registry.clone(), tid).unwrap();
            running.enter_guest();
            let mut original_visible = true;
            for _ in 0..scale {
                assert!(matches!(census.enter_with_pause_endpoint(
                    Some(context.thread().clone()), displaced.clone(), tid),
                    Err(GuestExecutorCensusError::DuplicateThread { .. })));
            ...
            work.insert(WorkMetric::HostBackendCalls, original_calls + rejected_calls).unwrap();
            observations.push(ContractObservation {
                ...
                implementation_revision: format!("sha256:{:x}", Sha256::digest(include_bytes!("guest_execution.rs"))),
                fixture_identity: "unit:duplicate-admission-live-endpoint".into(),
                scale: scale as u64,
                semantic_assertions: vec![
                    SemanticAssertion { name: "rejected_duplicate_preserves_running_owner".into(), passed: original_visible, detail: None },
                    SemanticAssertion { name: "drain_kicks_only_original_owner".into(), passed: original_calls == scale as u64 && rejected_calls == 0,
                        detail: Some(format!("original={original_calls}, rejected={rejected_calls}")) },
                ],
        ...
        if let Some(dir) = std::env::var_os("CARRICK_ADMISSION_RECEIPT_DIR") {
            ...
            std::fs::write(std::path::Path::new(&dir).join("observations.json"),
                serde_json::to_vec_pretty(&observations).unwrap()).unwrap();
        }
        evaluate(registry_contracts.require("kernel.mm.executor-admission").unwrap(), &observations).unwrap();
    }
