...
BEGIN { seconds=0; errors=0; seen=0; exited=0; self->active=0; self->gp=0; self->gt=0; self->asid=0; self->args=0; self->address=0; self->len=0; self->advice=0; self->total=0; self->zeroed=0; self->remapped=0; }
carrick*:::hvpatch-syscall-service-begin
/(pid == $target || progenyof($target)) && arg3 == 233/
{ errors=errors || self->active; self->active=1; self->gp=arg0; self->gt=arg1; self->asid=arg2; self->args=0; self->total=0; self->zeroed=0; self->remapped=0; seen=1; @counts["begin"]=count(); }
carrick*:::hvpatch-syscall-args
/(pid == $target || progenyof($target)) && arg0 == 233/
{ errors=errors || !self->active || self->args; self->args=1; self->address=arg1; self->len=arg2; self->advice=arg3; @counts["args"]=count(); }
