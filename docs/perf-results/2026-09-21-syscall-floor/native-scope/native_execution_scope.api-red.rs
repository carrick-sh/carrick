...
use carrick_conformance_contract::{Completeness, ContractId, ContractObservation, ContractRegistry, ExecutionLayer, SemanticAssertion, WorkMetric, WorkSnapshot, evaluate};
use carrick_hal::{NullGuestTimerBridge, NullHostSignalBridge, ThreadId};
use carrick_kernel::{dispatch::{CarrierBridges, SyscallDispatcher}, kernel::{CarrierProcess, objects::ExecutorId}};
use carrick_kernel_example::{driver::seed_initial_task_state, process::{AddressSpace, AsidAllocator, ExampleProcess}};
use std::{alloc::{GlobalAlloc, Layout, System}, cell::Cell, sync::Arc};
...
fn native_execution_scope_contract() {
    ...
    let bridges = CarrierBridges { host_signal: Arc::new(NullHostSignalBridge::default()), timers: Arc::new(NullGuestTimerBridge::default()) };
    let tid = ThreadId::from_guest_supplied_tid(carrick_abi::LINUX_BOOTSTRAP_PID as i32);
    let (process, context) = ExampleProcess::boot_root(tid.raw(), "native-scope-contract", bridges.host_signal.clone(), AddressSpace::allocate(&AsidAllocator::new()).unwrap()).unwrap();
    ...
    let mut execution = context.thread().claim_runnable(ExecutorId::for_transitional_thread(tid).unwrap()).unwrap();
    let mut executor = dispatcher.admit_native_executor(&context, &execution).unwrap();
    let interrupt = executor.interrupt_handle();
    drop(dispatcher.enter_native_execution(&mut executor, &context, &mut execution).unwrap());
    ...
    for scale in [1, 8, 32, 128] {
        ...
        ALLOCATIONS.set(Some(0));
        for _ in 0..scale {
            let scope = dispatcher.enter_native_execution(&mut executor, &context, &mut execution).unwrap();
        ...
        observations.push(ContractObservation {
            ...
            fixture_identity: "unit:native-execution-scope".into(), scale,
            semantic_assertions: vec![SemanticAssertion { name: "exact_completed_scopes_and_sticky_interrupt".into(), passed: exact && completed == scale, detail: Some(format!("completed={completed}")) }, SemanticAssertion::pass("allocator_positive_control_fired")],
    ...
    if let Some(dir) = std::env::var_os("CARRICK_NATIVE_SCOPE_RECEIPT_DIR") {
        ...
        std::fs::write(std::path::Path::new(&dir).join("observations.json"), serde_json::to_vec_pretty(&observations).unwrap()).unwrap();
    }
    drop(executor);
    context.thread().yield_from_executor(execution).unwrap();
    let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap().parent().unwrap();
    let registry = ContractRegistry::load(root).unwrap();
    evaluate(registry.require("kernel.mm.native-execution-scope").unwrap(), &observations).unwrap();
}
