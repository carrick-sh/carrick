...
import screen
out=root/'target/lease-cost/node-child-control'; screen.OUT=out; screen.ROOT=root
...
base=source.replace(worker,'')
echo="  const child = childProcess.execFileSync('/bin/echo', ['child-ok'], { encoding: 'utf8' });\n  assert.strictEqual(child, 'child-ok\\n');\n\n"
variants={'base':base.replace(child,''),'echo':base.replace(child,echo),'node':base,'standalone':"process.stdout.write('child-ok'); console.log('app-smoke ok');\n"}
...
manifest={'binary_sha256':hashlib.sha256(screen.BIN.read_bytes()).hexdigest(),'source_sha256':hashlib.sha256(source.encode()).hexdigest(),'image':screen.NODE,'diagnostic_only':True,'order':['base','echo','node','standalone'],'timing':'whole Node process, including teardown; file creation outside timed interval'}
(out/'inputs.json').write_text(json.dumps(manifest,indent=2)+'\n')
phase=sys.argv[1]
orders=[list(variants),list(reversed(variants)),['echo','standalone','base','node'],['node','base','standalone','echo']]
for block,order in [('warm',list(variants))]+list(enumerate(orders)):
 for name in order:
  label=f'childctrl-{phase}-{block}-{name}'
...
for name in variants:
 samples=[json.loads((out/f'childctrl-{phase}-{i}-{name}.jsonl').read_text())['guest_timing'][0][0] for i in range(4)]
