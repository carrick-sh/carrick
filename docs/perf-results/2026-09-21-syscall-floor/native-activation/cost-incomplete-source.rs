...
mod tests {
    ...
    fn native_data_activation_entry_cost() {
        ...
        for pages in [1u32, 8, 32, 128] {
            let id = 33_000 + pages as i32;
            let (kernel, root) = bootstrap(id);
            let fixture = real_production_cow_fixture(
                &kernel,
                &root,
                id + 200,
                0x9a00_a000_0000 + pages as u64 * 0x100_0000,
                0x9b00_a000_0000 + pages as u64 * 0x100_0000,
            ...
            let range = current
                .access_token()
                .write_range(GuestVa(TEST_VA), pages as usize * 4096)
                .unwrap()
                .unwrap();
            let mut prepared = authority
                .with_current_mutation(
            ...
            for sample in 0..9 {
                let (scope_ns, activated_ns) = if sample % 2 == 0 {
                    let base = run(false, ITERATIONS);
                    (base, run(true, ITERATIONS))
                } else {
                    let active = run(true, ITERATIONS);
                    (run(false, ITERATIONS), active)
                };
                println!(
                    "native_activation_cost {}",
                    serde_json::json!({
                        "pages": pages, "sample": sample, "iterations": ITERATIONS,
