    ...
    if std::env::args().skip(1).eq(["invalid-contract"]) {
        ...
        for scale in [1, 8, 32, 128] {
            ...
            for index in 0..scale {
                ...
                if index > 0 {
                    stable &= add_result == (add, add_errno)
                        && remove_result == (remove, remove_errno);
                }
                add_result = (add, add_errno);
                remove_result = (remove, remove_errno);
            }
            println!("scale={scale} add_ret={} add_errno={} remove_ret={} remove_errno={} stable={}",
                add_result.0, add_result.1, remove_result.0, remove_result.1, u8::from(stable));
            if !stable { std::process::exit(1); }
        }
