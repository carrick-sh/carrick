...
for index,arm in enumerate('ABBAABBA'):
    ...
    label=f'pristine-scrub-node-{index}-{arm}'
...
for name in ['node-app']:
    arms={a:[] for a in 'AB'}
    for index,arm in enumerate('ABBAABBA'):
        rows=[json.loads(x) for x in (out/f'pristine-scrub-node-{index}-{arm}.jsonl').read_text().splitlines()]
...
(out/'pristine-scrub-node-summary.json').write_text(json.dumps(results,indent=2)+'\n')
