...
for index,arm in enumerate('ABBAABBA'):
    ...
    label=f'retained-query-node-confirm-{index}-{arm}'
...
for name in ['node-app']:
    arms={a:[] for a in 'AB'}
    for index,arm in enumerate('ABBAABBA'):
        rows=[json.loads(x) for x in (out/f'retained-query-node-confirm-{index}-{arm}.jsonl').read_text().splitlines()]
...
(out/'retained-query-node-confirm-summary.json').write_text(json.dumps(results,indent=2)+'\n')
