...
for arm in ['base','scrub-candidate']:
 binary=root/'target/lease-cost'/('carrick-'+arm);rid='kill-floor-'+arm
 env=os.environ.copy();env['CARRICK_INSECURE_REGISTRIES']='localhost:5005,localhost:5050'
 cmd=[str(binary),'debug','lldb-run','--deadline-seconds','20','--out-dir',str(out/arm),'--run-id',rid,'--','--max-traps','18446744073709551615','--fs','host','--entrypoint','/usr/local/bin/python3',image,'-u','-c',code]
 with (out/(arm+'.log')).open('wb') as f:
  p=subprocess.run(cmd,env=env,stdout=f,stderr=subprocess.STDOUT)
 (out/(arm+'.json')).write_text(json.dumps({'argv':cmd,'returncode':p.returncode,'sha256':hashlib.sha256(binary.read_bytes()).hexdigest()},indent=2)+'\n')
