...
offset_after_fault = os.lseek(fd, 0, os.SEEK_CUR)
if offset_after_fault != 8192:
    failures.append("readv EFAULT advanced offset from 8192 to " + str(offset_after_fault))
assert lib.mprotect(p + 8192, 4096, 3) == 0
print("readonly_efault=" + ("ok" if offset_after_fault == 8192 else "FAIL offset=" + str(offset_after_fault)), flush=True)
