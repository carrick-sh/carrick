...
(out/'inputs.json').write_text(json.dumps({'argv':cmd,'run_id':rid,'iterations':1,'binary_sha256':hashlib.sha256(binary.read_bytes()).hexdigest(),'script_sha256':hashlib.sha256(script.read_bytes()).hexdigest()},indent=2)+'\n')
