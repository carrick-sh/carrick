...
impl HvfVmState {
    ...
    fn find_cow_inventory_extent(
        inventory: &HvpatchFrameInventory,
        compound_gpa: u64,
        compound_end: u64,
        owner_extent: Option<(u64, u64)>,
        mut visit: impl FnMut(),
    ) -> Option<((u64, u64), InventoryExtent)> {
        let contains = |base: u64, length: u64| {
            compound_gpa >= base
                && base
                    .checked_add(length)
                    .is_some_and(|end| compound_end <= end)
        };
        match owner_extent {
            Some((owner_base, owner_len)) => {
                let owner_end = owner_base.checked_add(owner_len)?;
                if compound_gpa < owner_base || compound_end > owner_end {
                    return None;
                }
                inventory
                    .extents
                    .range((owner_base, 0)..=(compound_gpa, u64::MAX))
                    .inspect(|_| visit())
                    .find(|((base, length), extent)| {
                        (extent.stage2_base, extent.stage2_length) == (owner_base, owner_len)
                            && base
                                .checked_add(*length)
                                .is_some_and(|end| end <= owner_end)
                            && contains(*base, *length)
                    })
    ...
    pub(crate) fn cow_inventory_split_shape(
        ...
        let (old_key, old) = Self::find_cow_inventory_extent(
            inventory,
            compound_gpa,
            compound_end,
            owner_extent,
