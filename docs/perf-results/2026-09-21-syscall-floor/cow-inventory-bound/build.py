...
root=Path('/Users/tjfontaine/.codex/worktrees/inotify-lease-cost/carrick'); out=root/'target/lease-cost/cow-inventory-bound'
p=root/'crates/carrick-vmm-hvf/src/trap/frame_inventory.rs'; candidate=p.read_text()
needle='            compound_end,\n            owner_extent,\n            || {},'
assert candidate.count(needle)==1
(out/'candidate-frame-inventory.rs').write_text(candidate)
env=os.environ.copy();env['RUSTC_WRAPPER']=''
try:
 for arm in ['control','candidate']:
  p.write_text(candidate.replace(needle,needle.replace('owner_extent,','None,')) if arm=='control' else candidate)
