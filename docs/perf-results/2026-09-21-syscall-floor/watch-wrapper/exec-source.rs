...
pub(crate) mod tests {
    ...
    fn invalid_inotify_resident_poll_cost() {
        measure_invalid_inotify_poll(false);
    }

    #[test]
    #[ignore = "explicit release-mode syscall cost diagnostic"]
    fn invalid_inotify_quantum_wrapper_cost() {
        measure_invalid_inotify_poll(true);
    }

    fn measure_invalid_inotify_poll(wrapped: bool) {
        let pid = 72_498;
        ...
        for sample in 0..22 {
            ...
            if sample > 0 {
                println!(
                    "resident_poll wrapped={wrapped} sample={} ns_per_pair={ns}",
                    sample - 1
                );
            }
        }
