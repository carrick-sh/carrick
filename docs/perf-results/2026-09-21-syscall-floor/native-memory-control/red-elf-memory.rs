...
mod tests {
    ...
    fn native_carrier_elf(words: &[u32], data_va: u64) -> Vec<u8> {
        ...
        for (i, base, len, flags) in [(0, 0x400000u64, words.len() * 4, 5u32), (1, data_va, 8, 6)] {
            ...
            for (offset, value) in [(8, 0x1000u64 * (i as u64 + 1)), (16, base), (32, len as u64), (40, len as u64), (48, 4096)] {
    ...
    fn native_carrier_elf_memory_control_round_trip() {
        use carrick_kernel::{
            ...
            dispatch::{DispatchOutcome, SyscallRequest, ThreadCtx,
        ...
        use native_syscall_slice::{Memory, native::{Code, State}};
        let (kernel, root) = bootstrap(37_000);
        let root_execution = execution_lease(&root, 37000);
        let tid = ThreadId::synthetic_for_tests(37_001);
        let fixture = real_production_cow_fixture(&kernel, &root, 37_001,
            0x9a00_c000_0000, 0x9b00_c000_0000, tid);
        let original = fixture.carrier.pin_original_data_for_test().unwrap();
        let dispatcher = carrick_kernel::dispatch::SyscallDispatcher::with_native_mm_for_test(fixture.dispatch_mm.clone());
        ...
        let current = fixture.child.current_mm(&execution).unwrap();
        let range = current.access_token().write_range(GuestVa(TEST_VA), 8).unwrap().unwrap();
        let mut prepared = authority.with_current_mutation(&current, tid, |mutation| {
            let mut cow = authority.break_foreign_cow(mutation, &current, range)?;
            Ok(fixture.child.borrow_current_native_data(&execution, &mut cow)?.prepare_for_execution())
        }).unwrap();
        drop(current);
        ...
        let mut mutator = dispatcher.mm_executor_census().enter_with_pause_endpoint(None,
            Arc::new(carrick_hal::GenericVcpuRegistry::new()), ThreadId::synthetic_for_tests(37002)).unwrap();
        let mut completed = 0;
        for scale in [1, 8, 32, 128] {
            for _ in 0..scale {
                ...
                let refusal = acquire_mutation_pause_for_test(fixture.dispatch_mm.pt_quiesce(), &mut mutator,
                    ThreadId::synthetic_for_tests(37002), fixture.child.shared().mm().id(),
                    dispatcher.mm_mutation_coordinator(), PtPauseBudget {
                        election: std::time::Duration::ZERO, drain: std::time::Duration::ZERO,
                    });
                assert!(matches!(refusal, Err(PtPauseError::TimedOut)));
                drop(refusal);
                assert!(scope.stop_requested());
                code.run(&image, &mut memory, &mut state, &mut |_, _| Ok(false)).unwrap();
                drop(active);
                drop(scope);
                assert_eq!(state.pc, image.entry() + 12);
                assert!(executor.memory_pause_pending());
                dispatcher.service_native_memory_control(&mut executor, &fixture.child, &execution).unwrap();
                assert!(!executor.take_stop_request());
                let outcome = dispatcher.dispatch_threaded_with_mm_executor(executor.dispatch_participation(), &fixture.child,
                    SyscallRequest::new(state.x[8], SyscallArgs::from([state.x[0], 0, 0, 0, 0, 0])),
                    &mut memory, &reporter, ThreadCtx::new(tid, &registry, &futex)).unwrap();
                assert!(matches!(outcome, DispatchOutcome::Errno { errno } if errno == carrick_abi::LINUX_EBADF));
                completed += 1;
                let current = fixture.child.current_mm(&execution).unwrap();
                let foreign = foreign_mm(&kernel, &root, &root_execution, fixture.child.task().key());
                let read = foreign.access_token().read_range(GuestVa(TEST_VA), 4).unwrap().unwrap();
                let mut actual = [0; 4];
                authority.read_foreign(&foreign, read, &mut actual).unwrap();
                drop(foreign);
                assert_eq!(u32::from_le_bytes(actual), u32::from_le_bytes(*b"same") + completed,
                    "ELF store did not reach carrier backing");
                authority.with_current_mutation(&current, tid, |_mutation| {
                    fixture.carrier.deny_native_data_for_test(TEST_VA, 6).unwrap();
                    Ok(())
                }).unwrap();
            }
            println!("native_carrier_elf scale={scale} completed_total={completed} close_errno=9 pause_requests_serviced={scale}");
        }
        assert_eq!(completed, 169);
        assert_eq!(original.prefix(), *b"same");
        let current = fixture.child.current_mm(&execution).unwrap();
        authority.with_current_mutation(&current, tid, |_mutation| {
            fixture.carrier.deny_native_data_for_test(TEST_VA, 0).unwrap();
            Ok(())
        }).unwrap();
        drop(current);
        let scope = dispatcher.enter_native_execution(&mut executor, &fixture.child, &mut execution).unwrap();
        assert!(prepared.activate(&scope).is_err(), "revoked carrier data was executable");
        drop(scope);
        drop(mutator);
        drop(executor);
        fixture.child.thread().yield_from_executor(execution).unwrap();
        assert_eq!(dispatcher.mm_executor_census().participant_count_for_probe(), 0);
