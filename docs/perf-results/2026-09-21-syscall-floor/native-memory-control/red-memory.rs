...
mod tests {
    ...
    fn native_admission_allows_carrier_cow_without_hardware_ack() {
        ...
        for scale in [1, 8, 32, 128] {
            ...
            let fixture = real_production_cow_fixture(
                &kernel, &root, 35_200 + scale,
                0x9a00_b000_0000, 0x9b00_b000_0000,
            ...
            for index in 1..scale {
                ...
                peers.push(kernel.reserve_thread_clone(&fixture.child, plan, None).unwrap()
                    .prepare(ThreadId::synthetic_for_tests(36_000 + index)).unwrap()
                    .commit().unwrap().into_context().unwrap());
            }
            let contexts: Vec<_> = std::iter::once(&fixture.child).chain(peers.iter()).collect();
            let mut leases: Vec<_> = contexts.iter().enumerate()
                .map(|(i, c)| execution_lease(c, 36000 + i as u64)).collect();
            let mut executors: Vec<_> = contexts.iter().zip(&leases)
                .map(|(c, e)| dispatcher.admit_native_executor(c, e).unwrap()).collect();
            ...
            fixture.stage1.begin_asid_load(resident).unwrap().mark_resident().unwrap();
            ...
            let result = authority.with_current_mutation(&current, fixture.child.thread().registry_id(), |mutation| {
                let mut cow = authority.break_foreign_cow(mutation, &current, range)?;
                Ok(fixture.child.borrow_current_native_data(&leases[0], &mut cow)?.prepare_for_execution())
            });
            assert!(result.is_ok(), "native census scale {scale} blocked real carrier COW: {:?}", result.as_ref().err());
            ...
            for ((executor, context), lease) in executors.iter_mut().zip(&contexts).zip(&mut leases) {
                let scope = dispatcher.enter_native_execution(executor, context, lease).unwrap();
            ...
            println!("native_carrier_cow scale={scale} completed={scale} hardware_ticket_retained=true");
        }
    }
