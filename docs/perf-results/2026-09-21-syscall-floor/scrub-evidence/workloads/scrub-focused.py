...
for index,arm in enumerate('ABBAABBA'):
    ...
    label=f'scrub-focused-{index}-{arm}'
...
for name in ['node-app','go-build-cold','python-os']:
    arms={a:[] for a in 'AB'}
    for index,arm in enumerate('ABBAABBA'):
        rows=[json.loads(x) for x in (out/f'scrub-focused-{index}-{arm}.jsonl').read_text().splitlines()]
...
(out/'scrub-focused-summary.json').write_text(json.dumps(results,indent=2)+'\n')
