...
out=root/'target/lease-cost/discard-edges/concurrency/trace1'
...
rid='edges-topology-1-20260922'
body='set -eu; for wave in 1 2 3 4 5; do pids=(); for ((i=0;i<1;i++)); do /opt/node-src/v24/out/Release/node /opt/nodejs-conformance/fixtures/app-smoke.js & pids+=("$!"); done; status=0; for child in "${pids[@]}"; do wait "$child" || status=1; done; test "$status" -eq 0; echo GROUP_DONE; done'
