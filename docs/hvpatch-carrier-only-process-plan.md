...
| **Teardown on Init Exit** | Calls `teardown(init_host_pid)` ([`supervisor.rs:103, 126, 235-262`](file:///Volumes/CaseSensitive/carrick/crates/carrick-runtime/src/namespace/supervisor.rs#L103)), invoking `libc::killpg(init_host_pid, SIGKILL)` and sweeping `region.members()` with `libc::kill(host, SIGKILL)`. | `destroy_persistent_vm_at_carrier_exit()` ([`carrier.rs::shutdown`](file:///Volumes/CaseSensitive/carrick/crates/carrick-runtime/src/carrier.rs)) and sibling thread cancellation in carrier. |
