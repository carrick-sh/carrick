...
**Mutable shared executable safety:** VMA metadata now records backing sharing independently of R/W/X permission and carries it through `mmap`, `mremap`, and fork snapshots. Native x86 classifies the complete planned instruction span under one metadata lock and permanently refuses block-cache publication, direct or pending edges, persistent cflow plans, and return-cache arm/hit whenever executable bytes overlap `MAP_SHARED` backing. This remains conservative for RX views because a second RW view, `pwrite(2)`, or another process can mutate the same bytes without entering the RX VMA's local write hooks. The mandatory `shared-exec-mutation-x86_64-linux` fixture verifies all three mutation paths, including a pipe-synchronized fork child.
...
## Effective intended-harness ceiling
