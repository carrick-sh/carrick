...
- **Current strict census:** **411/428 (96.0%)** on FreeBSD/amd64. The checked-in
...
- **Standalone runtime backlog:** 6 probes: three live-protection/fault-retry
  cases, two vfork cases, and FreeBSD's missing datagram ICMP socket.
...
1. Design a coordinated protection/fault-retry contract: enforce live host
   ...
   `roprotect`).
2. Implement real `CLONE_VM|CLONE_VFORK` address-space sharing and parent
   suspension (`vforkvmshare`, `vforkexecthread`).
3. Add a Linux-compatible datagram ICMP implementation rather than relying on
   FreeBSD, which returns `EPROTONOSUPPORT` for `SOCK_DGRAM/IPPROTO_ICMP`.
4. Run the paired network scenarios under their intended harness and continue the
...
## Session summary: 245 → 411 (+166 net)
...
backend-neutral dispatcher coverage. **411/428 = 96.0% strict standalone.**

The 17 non-OK rows split cleanly:
...
- **Host capability (1):** `icmp` (FreeBSD lacks Linux datagram ping sockets).
...
The strict standalone result is **411/428**. Eleven client/server binaries need
the coordinated network harness for a meaningful verdict; the other six
