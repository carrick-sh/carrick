...
| `bridge_net_identity` | Compose smoke checks Docker-shaped bridge identity across `getifaddrs`, `gethostname`, `uname`, `/proc/sys/kernel/hostname`, `/proc/net/dev`, `/proc/net/route`, `/sys/class/net`, rtnetlink link/address/route dumps, `/etc/hosts`, `/etc/hostname`, `/etc/resolv.conf`, generated host-gateway names, explicit `ExtraHosts` host-gateway overrides, and user-provided DNS/search/options ordering. Prevents the runtime Linux network model from drifting between UTS identity, procfs, sysfs, rtnetlink, resolver, and hosts renderers. |
...
| `udp_published_server` + `udp_published_client` | Compose smoke sends UDP between same-bridge services and through a host-published UDP port, asserting the same-bridge packet reaches the server from a non-loopback bridge address and the host-published packet reaches the server from the bridge gateway. Prevents bridge UDP publish support from accepting `/udp` metadata while dropping guest replies or leaking raw loopback proxy source addresses into the guest. |
...
| **A stopped child is not exit-ready for WEXITED-only waitid/pidfd waiters, and namespace-member SIGCONT uses real host delivery so a stopped target resumes before the final reap** | 🧪 `io_wait::tests::child_status_ready_ignores_stopped_child` + 🧪 `signal::tests::namespace_member_xsig_policy_routes_catchable_standard_signals` | Go `os/exec.TestWaitid` / `go-os_exec` |
...
| **large blocking host-pipe writes that make partial progress hand off a pinned continuation when the pipe fills, releasing the dispatcher so sibling close/error cleanup can run while preserving Linux's signal-interrupted short-count behavior** | 🧪 `dispatch::overlay_dispatch_tests::large_blocking_host_pipe_write_hands_off_after_partial_progress` | CPython `test_crash_big_data` / gh-94777 broken process-pool cleanup; `blockingpipewrite` |
...
| **preadv2 with `RWF_HIPRI` over a regular host-backed file treats the flag as an advisory priority hint and reads like `preadv` (return count and iovec contents match Linux)** | ✅ `preadv2flags` | preadv202/preadv202_64 |
...
| **openat2 open_how validation: normal O_RDWR\|O_CREAT open succeeds (flags+mode passed through, no longer whitelisted); mode-without-O_CREAT / mode>0o7777 / unknown-resolve-bits / size<sizeof → EINVAL; size>sizeof zero-pad accepted, nonzero-pad→E2BIG; bad dirfd→EBADF** | ✅ `openat2valid` | openat201, openat203 |
| **openat2 RESOLVE_* path-walk restrictions: resolve=0 controls still follow symlinks and permit dirfd escapes, while `RESOLVE_NO_SYMLINKS`/`NO_MAGICLINKS` reject symlink and proc-fd magic-link walks with ELOOP, `RESOLVE_BENEATH`/`NO_XDEV` reject escapes or VFS mount crossings with EXDEV, and `RESOLVE_IN_ROOT` makes an absolute path open relative to the dirfd root** | ✅ `openat2resolve` | openat202 |
...
| Syscall writes into a dynamic `mprotect(PROT_READ)` mapping return `EFAULT`, while syscall reads from the same mapping still succeed and `mprotect(PROT_READ\|PROT_WRITE)` clears the gate | ✅ `roreadwrite` | mprotect/read-write buffer permission semantics |
