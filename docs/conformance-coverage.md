...
| `bridge_net_identity` | Compose smoke checks Docker-shaped bridge identity across `getifaddrs`, `/proc/net/dev`, `/proc/net/route`, `/sys/class/net`, rtnetlink link/address/route dumps, `/etc/hosts`, `/etc/resolv.conf`, generated host-gateway names, explicit `ExtraHosts` host-gateway overrides, and user-provided DNS/search/options ordering. Prevents the runtime Linux network model from drifting between procfs, sysfs, rtnetlink, resolver, and hosts renderers. |
