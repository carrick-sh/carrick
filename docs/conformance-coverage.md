...
| **openat2 RESOLVE_* path-walk restrictions: resolve=0 controls still follow symlinks and permit dirfd escapes, while `RESOLVE_NO_SYMLINKS`/`NO_MAGICLINKS` reject symlink and proc-fd magic-link walks with ELOOP, `RESOLVE_BENEATH`/`NO_XDEV` reject escapes or VFS mount crossings with EXDEV, and `RESOLVE_IN_ROOT` scopes absolute paths under the dirfd root** | ✅ `openat2resolve` | openat202 |
