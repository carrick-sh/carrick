...
| **Populations** — eight distinct thread sets, all `usize` | delete `count() -> usize`, replace with a witness type | **Not built.** `fn count(&self) -> usize` is still a trait method at `carrick-hal/src/threaded.rs:213`. |
| **Lifecycle** — "alive" vs. per-purpose "can reach a safe point" | explicit run state on kernel `Thread` | **Not built.** The prescribed shape did land once, in `kernel/crash_capture.rs` (`CrashQuorum`, `CrashQuorumPoll`), and the document records that it caught two over-count cases the audit itself had missed. Copy it; do not invent a second. |
