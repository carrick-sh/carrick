...
- **Enforcement & Non-leakage**: Permits and guards cannot be constructed freely, cloned, copied, leaked as raw mutex guards, or used across mismatched owner identities. Linear borrow-bound semantics are proven by type-level assertions (`assert_not_impl_any!`). Sibling modules outside `dispatch::sysv` cannot access raw/scoped lock helpers or construct permits.
- **Call-Site Migration & Same-Shmid Self-Deadlock Prevention**: All five production paired sites in `dispatch/sysv.rs` (`note_sysv_remap_file_pages`, `commit_sysv_fork_inheritance`, `commit_remapped_shmat`, `validate_shmdt`/`commit_shmdt`, and `commit_host_alias_shmat`) have been migrated to the typed authority. Same-shmid remap handles reservation cancellation before acquiring paired authority, completely eliminating recursive mutex acquisition during `PendingShmat::drop`. Verified with bounded subprocess watchdog regression testing (`remapped_shmat_same_shmid_does_not_deadlock_and_preserves_accounting`), proving pre-fix commit `560ce940e` RED (watchdog timeout) and fixed GREEN (0.01s, `nattch == 1`, `pending_attaches == 0`).
- **Ergonomic Standalone APIs & Scoped Visibility**: Standalone namespace-only and process-only accessors remain clean via private scoped closures (`with_state`, `with_state_mut`, `with_sysv_process`, `with_sysv_process_mut`). `SysvIpcNamespace.state` and `SyscallDispatcher.sysv_process` are strictly private, and all raw/scoped lock helpers are private to `dispatch::sysv` (`pub(in crate::dispatch::sysv)`), structurally preventing sibling modules from expressing reverse acquisition.
- **Deterministic SETALL Witness & Revalidation**: `sysv_semctl` for `LINUX_SETALL` employs a typed `SetallPrepareWitness` across the guest memory read window; finalize revalidates exact semaphore set identity, generation/scan index, non-removal, and write permissions before mutating state. Verified with deterministic interleaving pause hooks testing permission revocation (`LINUX_EACCES`), removal (`LINUX_EIDRM`), and valid success control.
- **Fail-Closed Static API & Lock Authority Gate**: Added `scripts/migrate/check-dispatch-lock-authority.py` and `scripts/migrate/dispatch-lock-authority.json` (22 self-tests in `--self-test`), verifying exact 107 raw lock sites, monotone category ceilings, trusted minting/paired classifications, exact visibility tokens (`pub(in crate::dispatch::sysv)`), and strict cross-module caller exclusion across all runtime source files.
- **Subprocess Watchdog Concurrency Verification**: Added bounded subprocess regression testing with watchdog timeouts. The race enters the production `synthetic_proc_context` route while attachment cleanup and paired mutation run concurrently; an intentionally wedged child proves timeout kill and reap.
