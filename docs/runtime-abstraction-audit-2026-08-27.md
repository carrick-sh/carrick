...
- **Enforcement & Non-leakage**: Permits and guards cannot be constructed freely, cloned, copied, leaked as raw mutex guards, or used across mismatched owner identities. Comprehensive compile-fail tests compile against the actual authority API and reject direct construction, multiple minting, cloning, lifetime escape, arbitrary namespace arguments, module visibility leaks, and reverse acquisition attempts.
- **Call-Site Migration & Self-Deadlock Prevention**: All five production paired sites in `dispatch/sysv.rs` (`note_sysv_remap_file_pages`, `commit_sysv_fork_inheritance`, `commit_remapped_shmat`, `validate_shmdt`/`commit_shmdt`, and `commit_host_alias_shmat`) have been migrated to the typed authority. Same-shmid remap handles reservation rollback before acquiring paired authority, completely eliminating recursive mutex acquisition during `PendingShmat::drop`.
...
- **SETALL Witness & Concurrency Revalidation**: `sysv_semctl` for `LINUX_SETALL` employs a typed `SetallPrepareWitness` across the guest memory read window; finalize revalidates exact semaphore set identity, generation/scan index, non-removal, and write permissions before mutating state.
- **Fail-Closed Gate**: Added `scripts/migrate/check-dispatch-lock-authority.py` and `scripts/migrate/dispatch-lock-authority.json`, with self-tests (`--self-test`) and shrink-only enforcement wired into `just lint-domains`. The scanner handles parenthesized expressions, local let aliases, extended timed lock methods (`try_lock_for`, `try_lock_until`), and production code under `#[cfg(any(test, ...))]`. The exact inventory enforces monotone category ceilings and validates trusted minting and paired boundary classifications.
- **Concurrency Verification**: Added bounded subprocess regression testing with watchdog timeouts. The race enters the production `synthetic_proc_context` route while attachment cleanup and paired mutation run concurrently; an intentionally wedged child proves timeout kill and reap. Added concurrent `SETALL` permissions toggling and same-shmid remap accounting preservation tests.
