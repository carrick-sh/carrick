...
- `carrick dispatch-syscall <nr> --args ...` exercises the host-side syscall
  ...
  `read(2)`, `readv(2)`, `pread64(2)`, `write(2)`, `writev(2)`, `close(2)`,
...
- Linux ABI outputs for `stat`, `statfs`, `getdents64`, `iovec`,
  `eventfd` counters, `epoll_event`, `winsize`, `timespec`, `timeval`,
  `timezone`, auxv entries, `utsname`, `rlimit`, and signal-action stubs are
