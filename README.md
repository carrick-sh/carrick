...
- Dynamic-linker bring-up syscalls now include bootstrap `uname(2)`, `getpid(2)`
  ...
  capability probes, scheduler affinity/yield probes, `personality(2)`, and
  minimal `rt_sigaction(2)`/`rt_sigprocmask(2)` stubs.
...
- `scripts/build-linux-fixtures.sh` builds static Linux/aarch64 Rust fixtures
  ...
  `membarrier(2)` queries, scheduler affinity/yield calls, `rseq(2)` fallback,
  `flock(2)`, `sendfile(2)`, `preadv(2)`, `read(2)`, `close(2)`, and `exit(2)`,
  giving the loader, HVF loop, rootfs, and dispatcher a tight feedback loop.
