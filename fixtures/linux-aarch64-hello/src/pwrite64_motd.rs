...
pub extern "C" fn _start() -> ! {
    unsafe {
        ...
        if stdout_pwrite != ESPIPE {
            exit(10);
        }
        let stderr_pwrite = syscall4(
            SYS_PWRITE64,
            2,
            PAYLOAD.as_ptr() as u64,
            PAYLOAD.len() as u64,
        ...
        if stderr_pwrite != ESPIPE {
            exit(11);
        }
        let bad_pwrite = syscall4(
            SYS_PWRITE64,
            999,
            PAYLOAD.as_ptr() as u64,
            PAYLOAD.len() as u64,
