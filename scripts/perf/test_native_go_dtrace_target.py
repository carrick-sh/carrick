...
class NativeGoDtraceTargetTests(unittest.TestCase):
    ...
    def test_store_controls_are_validated_and_receipt_bound(self) -> None:
        captured_environment: dict[str, str] = {}
        ...
        with tempfile.TemporaryDirectory() as temporary:
            store = pathlib.Path(temporary) / "active-store"
            store.mkdir()
            receipt = {
                "schema": "carrick.sparse-store-tree.v1",
                "root": str(store.resolve()),
                "tree_sha256": "a" * 64,
                "entries": [],
                "validator": {"sha256": "b" * 64},
            }
            output = io.StringIO()
            with (
                mock.patch.object(
            ...
            self.assertEqual(
                validate.call_args.kwargs["validator"],
                native_go_dtrace_target.REPO
                / "target/release/examples/native_manifest_census",
            )
            self.assertEqual(
                captured_environment["CARRICK_DSR_STORE_DIR"],
                str(store.resolve()),
            )
            self.assertEqual(
                captured_environment["CARRICK_DSR_STORE_AUGMENTATION"],
                "0",
            )
            provenance = json.loads(
                output.getvalue().strip().removeprefix("TARGET_PROVENANCE=")
            )
            self.assertEqual(provenance["store"], receipt)
            self.assertEqual(
                provenance["environment_overlay"]["CARRICK_DSR_STORE_DIR"],
                str(store.resolve()),
            )
            self.assertEqual(
                provenance["environment_overlay"][
    ...
    def test_store_augmentation_on_unsets_the_exact_opt_out(self) -> None:
        captured_environment: dict[str, str] = {}
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(provenance["store_augmentation"], "on")

    def test_store_control_refuses_missing_or_ambient_mismatched_path(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            missing = pathlib.Path(temporary) / "missing-store"
            argv = [
                "native_go_dtrace_target.py",
                "--run-id",
                "store-refusal-test",
            ...
            store = pathlib.Path(temporary) / "active-store"
            store.mkdir()
            argv[argv.index(str(missing))] = str(store)
            with (
                mock.patch.object(sys, "argv", argv),
                mock.patch.dict(
                    os.environ,
                    {"CARRICK_DSR_STORE_DIR": str(missing)},
                    clear=True,
                ),
                mock.patch.object(
                    native_go_build,
                ...
                mock.patch.object(native_go_dtrace_target.subprocess, "Popen") as launch,
                self.assertRaisesRegex(RuntimeError, "ambient Carrick controls"),
            ):
                native_go_dtrace_target.main()
            ...
            with (
                ...
                mock.patch.dict(
                    os.environ,
                    {"CARRICK_UNDECLARED_STORE_CONTROL": "1"},
                    clear=True,
                ),
                mock.patch.object(
                    native_go_build,
                ...
                native_go_dtrace_target.main()
            unknown_launch.assert_not_called()
