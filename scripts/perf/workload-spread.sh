...
ORDER=(startup compute fs-walk build-cold build-warm)

run() { # engine workload-name sample
  local eng=$1 name=$2 i=$3 rid="spread-$1-$2-$$-$3" out
  local script="set -eu; ${WL[$name]}"
  if [[ $eng == carrick ]]; then
    out=$(target/release/carrick run --exec-backend native -e "CARRICK_RUN_ID=$rid" -w /tmp "$IMAGE" /bin/sh -c "$script" 2>/dev/null) || return 1
  else
...
prime() {
  ...
  if [[ $eng == carrick ]]; then
    target/release/carrick run --exec-backend native -e "CARRICK_RUN_ID=$rid" -w /tmp "$IMAGE" /bin/sh -c "$s" >/dev/null 2>&1 || true
  else
