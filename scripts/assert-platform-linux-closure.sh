...
# applevisor, nor applevisor-sys are in the build dependency closure of carrick-cli.

TREE_OUTPUT=$(cargo tree --no-default-features --features platform-linux --target aarch64-unknown-linux-gnu -p carrick-cli -e no-dev)
