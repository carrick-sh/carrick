...
CEOF
    ...
    # 37) posix-timer-fork: a parent timer_create()s a POSIX timer then forks; the
    #     CHILD must see EINVAL on timer_getoverrun() of the inherited id (POSIX: a
    #     fork child inherits NO timers) and print child-clean, while the parents
    #     own timer stays live. Locks the KVM fork-clear path:
    #     host_signal::reinit_after_fork clears the process-global carrick-timer-core
    #     POSIX registry in the child so it does not reuse the parents timer ids.
    #     Needs glibc timer_create/fork -> the REAL dispatcher. REQUIRED gate.
    ptfork_src="$fixdir/posix-timer-fork/posix-timer-fork.c"
    gcc -static -O2 -o /tmp/posix_timer_fork "$ptfork_src"
    # -s KILL: a regression that wedges must be force-killed so the smoke fails
    # fast rather than hanging for the full timeout.
    got="$(timeout -s KILL 30 sg kvm -c "$kvm run-elf /tmp/posix_timer_fork" 2>/dev/null)" && code=0 || code=$?
    if [ "$got" = "child-clean" ] && [ "$code" -eq 0 ]; then
      echo "OK [real-dispatch+posix-timer-fork]: a forked child does not inherit the parent POSIX timer ids — reinit_after_fork clears the registry."
    else
    ...
    # 41) mapfixed-private: MAP_FIXED|MAP_PRIVATE fork-privacy (Task 9). A guest
    #     mmaps two contiguous SHARED-aperture pages, then mmap(MAP_FIXED|
    #     MAP_PRIVATE|MAP_ANON)s page 0 over itself to make it PRIVATE while page 1
    #     stays SHARED as the cross-process signal. The parent writes 0x42 to page
    #     0, forks, the child writes 0x99 to page 0 then sets the SHARED page 1; the
    #     parent waits on page 1 (which propagates), reaps the child, and re-reads
    #     page 0. On a correct (PRIVATE) repoint the child write was COW-isolated so
    #     the parent still sees its own 0x42 -> "private-ok"; a LEAK to the shared
    #     backing shows the child 0x99 -> exit 3. The carrick shared aperture is host-
    #     MAP_SHARED (inherited across fork + visible to siblings), so the MAP_FIXED|
    #     MAP_PRIVATE must REPOINT the page-0 stage-1 leaf to a per-process PRIVATE
    #     overlay. Before Task 9 KvmTrapEngine::repoint_private fell through to the
    #     trait DEFAULT no-op, so the leaf was never redirected and the child write
    #     leaked back to the parent (exit 3). The override seeds the boot-mapped
    #     overlay slot then pt_edit_and_flush(map_aliased) repoints the leaf to the
    #     overlay IPA. Unlike mprotect-ro this is a FORK-PRIVACY test (not a stale-
    #     TLB test), so the nested-KVM lane does NOT mask it -- a clean RED->GREEN.
    #     Needs glibc mmap/fork/waitpid -> the REAL dispatcher. REQUIRED gate.
    mapfixedpriv_src="$fixdir/mapfixed-private/mapfixed-private.c"
    gcc -static -O2 -o /tmp/mapfixed_private "$mapfixedpriv_src"
    # -s KILL: a regression that wedges (the repoint corrupts the in-flight mmap)
    # must be force-killed so the smoke fails fast rather than hanging.
    got="$(timeout -s KILL 30 sg kvm -c "$kvm run-elf /tmp/mapfixed_private" 2>/dev/null)" && code=0 || code=$?
    if [ "$got" = "private-ok" ] && [ "$code" -eq 0 ]; then
      echo "OK [real-dispatch+mapfixed-private]: a MAP_FIXED|MAP_PRIVATE over a shared-aperture VA was repointed to a per-process overlay -- a forked-child private write stayed COW-isolated (no leak through the shared backing)."
    else
      printf "FAIL [mapfixed-private]: stdout=[%s] exit=%s oracle=[private-ok]\n" "$got" "$code" >&2
      exit 1
    fi

    # 42) mapfixed-private-syscall: repoint_private SYSCALL-buffer correctness. A
    #     guest mmaps a SHARED-aperture page, memsets it 0xAA, then mmap(MAP_FIXED|
    #     MAP_PRIVATE|MAP_ANON)s the same VA to repoint its stage-1 leaf to a
    #     per-process PRIVATE overlay (608 GiB) and memsets it 0xBB. The guest OWN
    #     EL0 stores hit the overlay (mapfixed-private already locks that). This
    #     case locks the SYSCALL path: it write(2)s the page down a pipe (the
    #     read_bytes copy) and read(2)s back into it (the write_bytes copy), and
    #     asserts the syscall saw the PRIVATE 0xBB/0xCC, never the stale shared
    #     0xAA -> "syscall-priv-ok"; a syscall copy resolving to the shared backing
    #     exits 3. The bug: carrick syscall read/write treated the guest pointer
    #     as a GPA directly (identity VA==GPA) WITHOUT stage-1 translating, and the
    #     shared aperture (576 GiB) is BELOW the 1-TiB high-VA translate threshold,
    #     so the copy resolved to the shared region the VA still covers. The fix
    #     stage-1-translates a syscall buffer iff needs_stage1_translation(va,len)
    #     (high-VA OR shared-aperture), keeping PROT_NONE keyed on the VA. Needs
    #     glibc mmap/pipe/read/write -> the REAL dispatcher. REQUIRED gate.
    mapfixedprivsys_src="$fixdir/mapfixed-private-syscall/mapfixed-private-syscall.c"
    gcc -static -O2 -o /tmp/mapfixed_private_syscall "$mapfixedprivsys_src"
    got="$(timeout -s KILL 30 sg kvm -c "$kvm run-elf /tmp/mapfixed_private_syscall" 2>/dev/null)" && code=0 || code=$?
    if [ "$got" = "syscall-priv-ok" ] && [ "$code" -eq 0 ]; then
      echo "OK [real-dispatch+mapfixed-private-syscall]: a syscall read/write buffer at a repoint_private overlay VA was stage-1-translated to the PRIVATE overlay backing -- the syscall saw the guest private bytes, not the stale shared aperture."
    else
