#!/usr/sbin/dtrace -qs
/*
 * Count stage-1 maintenance requests by caller on the retained Node artifact.
 * Derived from the qualified invalidation-scope probe. ustack(16) adds HIGH
 * perturbation. Counts identify candidates only, never removable wall time. Scalar ABI qualified from
 ...
 * Scalar events is a presence flag: increments race between CPUs. Exact
 * totals come from count() aggregations, cross-checked against caller sums.
...
dtrace:::END
{
    printf("COWTLBI|summary|probe_seen=%d|errors=%d|seen=%d|code=%d|bounded=%d\n", events, errors, seen, code, bounded);
