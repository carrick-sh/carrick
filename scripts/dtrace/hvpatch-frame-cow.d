 ...
 * carrick*:::hvpatch-frame-cow-intent carries the typed write authority
...
dtrace:::BEGIN
{
    ...
    fault_ttbrs = 0;
    faults = 0;
    printf("HVPATCHFRAMECOW3|header|version=3\n");
}
...
{
    faults++;
    printf("HVPATCHFRAMECOW3|fault|host_pid=%d|esr=%x|elr=%x|far=%x|insn=%x|rn=%d|xrn=%x\n",
        pid, arg0, arg1, arg2, arg3, (uint32_t)arg4, arg5);
}
...
{
    fault_ptes++;
    printf("HVPATCHFRAMECOW3|fault_pte|host_pid=%d|va=%x|l0=%x|l1=%x|l2=%x|l3=%x\n",
...
{
    fault_ttbrs++;
    printf("HVPATCHFRAMECOW3|fault_ttbr|host_pid=%d|va=%x|ttbr0=%x\n",
        pid, arg0, arg1);
}
...
/(pid == $target || progenyof($target))/
{
    ...
    errors += (arg1 & 0x800) == 0;
    printf("HVPATCHFRAMECOW3|pte|ts=%d|host_pid=%d|va=%x|leaf=%x|expected_ipa=%x|expected_ap=%x|phase=%d\n",
...
/(pid == $target || progenyof($target))/
{
    ...
    printf("HVPATCHFRAMECOW3|fork_frame|ts=%d|host_pid=%d|linux_pid=%d|linux_tid=%d|mm=%d|asid=%d|kind=%d|parent_mapping=%d|child_mapping=%d|frame=%d|ipa=%x|length=%x|identity=%d\n",
...
/(pid == $target || progenyof($target))/
{
    ...
    vm_events++;
    printf("HVPATCHFRAMECOW3|vm|ts=%d|host_pid=%d|operation=%d|admission=%d|generation=%d\n",
...
/(pid == $target || progenyof($target))/
{
    ...
    printf("HVPATCHFRAMECOW3|stage2|ts=%d|host_pid=%d|vm=%d|phase=%d|ipa=%x|length=%x|host=%x|perms=%x\n",
...
/(pid == $target || progenyof($target))/
{
    ...
    errors += arg1 == arg2;
    errors += arg3 == arg4;
    printf("HVPATCHFRAMECOW3|event|ts=%d|host_pid=%d|linux_pid=%d|linux_tid=%d|mm=%d|asid=%d|intent=%d|phase=%d|va=%x|old_frame=%d|new_frame=%d|old_ipa=%x|new_ipa=%x|identity=%d\n",
...
/(pid == $target || progenyof($target))/
{
    ...
    errors += arg2 != arg3;
    errors += arg4 != 16384;
    printf("HVPATCHFRAMECOW3|copy|ts=%d|host_pid=%d|old_frame=%d|old_ipa=%x|source_hash=%x|dest_hash=%x|length=%d\n",
...
dtrace:::END
{
    printf("HVPATCHFRAMECOW3|summary|events=%d|identities=%d|intents=%d|fork_identities=%d|fork_frames=%d|stage2_maps=%d|stage2_unmaps=%d|vm_events=%d|ptes=%d|copies=%d|faults=%d|fault_ptes=%d|fault_ttbrs=%d|errors=%d|drops=%d|bounded=%d|target_exited=%d\n",
        events, identities, intents, fork_identities, fork_frames, stage2_maps,
