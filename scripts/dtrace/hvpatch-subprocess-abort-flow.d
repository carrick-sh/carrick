/*
 * Abort-context syscall/fault diagnostic, derived from hvpatch-guest-syscall-flow.d.
 * Captures signal syscalls for all tasks and every syscall for observed failing
 * Linux tid96. Fault ABI source-qualified: syndrome,ELR,FAR,pid,tid.
 * Diagnostic perturbation only; syscall probes fire for all tasks even when filtered.
 *
...
/(pid == $target || progenyof($target)) && (uint64_t)arg0 == self->nr && (self->ltid == 96 || (self->nr >= 129 && self->nr <= 139))/
{
...
/(pid == $target || progenyof($target)) && (self->ltid == 96 || ((uint64_t)arg0 >= 129 && (uint64_t)arg0 <= 139))/
{
    returns++;
...
/pid == $target || progenyof($target)/
{
 printf("ABORTFAULT|ts=%llu|esr=0x%llx|elr=0x%llx|far=0x%llx|lpid=%d|ltid=%d\n", timestamp, (uint64_t)arg0, (uint64_t)arg1, (uint64_t)arg2, (int32_t)arg3, (int32_t)arg4);
}
