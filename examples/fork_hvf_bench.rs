...
fn main() {
    ...
    unsafe {
        ...
        let mut bits = 0u32;
        sys::hv_vm_config_get_max_ipa_size(&mut bits);
        sys::hv_vm_config_set_ipa_size(cfg, bits);
        ...
        for i in 0..(page_sz / 4) {
            *code.add(i) = 0xd4000001;
        } // svc #0
        assert_eq!(
            sys::hv_vm_map(
                code as *mut _,
                0x4000,
                page_sz,
                sys::HV_MEMORY_READ | sys::HV_MEMORY_EXEC
            ),
            0
        );

        let mut vcpu = 0u64;
        let mut exit: *const sys::hv_vcpu_exit_t = std::ptr::null();
        assert_eq!(
            sys::hv_vcpu_create(&mut vcpu, &mut exit, sys::hv_vcpu_config_create()),
            0
        );
        ...
        // COLD: first run.
        let t = Instant::now();
        let r = sys::hv_vcpu_run(vcpu);
        let cold = t.elapsed();
        println!(
            "cold hv_vcpu_run: {:.3} ms (ret={r:#x})",
        ...
        for _ in 0..runs {
            sys::hv_vcpu_set_reg(vcpu, sys::hv_reg_t::PC, 0x4000);
            let t = Instant::now();
            sys::hv_vcpu_run(vcpu);
            warm += t.elapsed();
        }
        println!(
            "warm hv_vcpu_run avg: {:.4} ms (n={runs})",
            warm.as_secs_f64() * 1e3 / runs as f64
        );
    }
}
